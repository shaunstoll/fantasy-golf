# Fantasy Golf

Next.js (App Router) fantasy golf scoring app deployed on Vercel.

## Stack

- **Framework**: Next.js 16 with Turbopack, React 19, TypeScript 6
- **API**: tRPC v11 (single `tournament.get` endpoint)
- **Styling**: Tailwind CSS v4
- **State**: Zustand (favorite teams persisted to localStorage)
- **Testing**: Vitest 4 + Testing Library (unit), Playwright (e2e)
- **Linting/Formatting**: oxlint + oxfmt, enforced via Husky pre-commit

## Architecture

Clients (`src/clients/`) fetch tournament data → ScoringService (`src/services/scoring.service.ts`) computes fantasy standings → tRPC router returns `Standing[]` → frontend polls every 5s.

Team rosters are stored as JSON in `data/{year}/{tournament}.json` and loaded via `src/db/teams.ts` based on the `NEXT_PUBLIC_TOURNAMENT` env var. Run `npm run get-teams -- <tournament>` to scrape rosters from the leaderboard API.

## Commands

- `npm run dev` — start dev server
- `npm run typecheck` — TypeScript check
- `npm run lint` / `npm run lint:fix` — oxlint
- `npm run fmt` / `npm run fmt:check` — oxfmt
- `npm test` — run Vitest
- `npm run e2e` — run Playwright
- `npm run get-rankings` — scrape player rankings
- `npm run get-teams` — scrape team rosters

## Workflow

- Solo project. Commit and push directly to `prod` — no feature branches, no PRs.
- Experimental vibe-coding repo. The owner reviews **intent, not code** — they won't read diffs. Confirm intent when it's genuinely ambiguous, but otherwise implement, verify (fmt/lint/typecheck/test), and push directly to `prod`.

## Rules

- Never use type `any`
- Player matching is by exact `"FirstName LastName"` string — be careful with name formatting
- Scoring rules live in one place: `src/services/scoring-rules.ts` (`computePlayerPoints`). Both `getStandings` and `getLeaderboard` must use it — never recompute point categories inline (they drifted once and caused a bonus bug).
- Bonus dots and the header legend are driven by `src/config/bonuses.ts` (color + label + description). Add/edit a bonus there, not in the components.
- Player nationalities may arrive as ISO alpha-3 codes; always build flag URLs via `flagCode()` (`src/utils/nationality.utils.ts`) since datagolf serves IOC codes.
- Datagolf tee times (a `t`/thru value containing a colon) are **event-local wall-clock** strings with no am/pm. The feed has no timezone field, but `info.times` gives each round's first/last tee as UTC instants; `src/utils/tee-time.utils.ts` derives the venue's UTC offset from those anchors and converts tee times to UTC (`teeTimeUtc`), which the UI renders in the viewer's local timezone. Never hardcode a tournament's timezone — PGA/US Open venues change coasts year to year.
- Loading skeletons live in `src/components/loaders/` (`standings.skeleton.tsx`, `leaderboard.skeleton.tsx`). When a change alters a row/column/header layout enough that the skeleton would misrepresent it, update the matching skeleton to keep it in sync.
- Authoritative scores live on the fantasy-golf-neon Vercel app: `/api/leaderboard` (current event totals) and `/api/overall` (season per-major + combined total). Cross-check our totals against these after a freeze or WD/backup change — match teams on a normalized name key (spellings differ per endpoint). See the `freezing-tournament-results` skill.
