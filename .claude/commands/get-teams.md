...
2. **Scrape the rankings:** `npm run get-rankings -- $1`
   - This pulls DataGolf's current world rankings and writes them to `data/{currentYear}/$1-rankings.json`. The **leaderboard view** (`scoring.service.getLeaderboard`) scores its ranking bonuses off this file (via `allRankings` in `src/db/rankings.ts`), so it must exist and reflect the rankings as of _this_ event — world rankings shift week to week, so a stale file from a prior major gives wrong bonuses.
   - Run this in the same sitting as step 1 so the roster ranks (baked into `$1.json`) and the leaderboard ranks (`$1-rankings.json`) agree. The rankings page isn't event-specific, but you still want the snapshot taken at this tournament's time.

3. **Check for name mismatches:** the script prints `No ranking found for player: <Name>` to stderr for any roster name that didn't match a DataGolf ranking. Player matching is exact `"FirstName LastName"` (see CLAUDE.md). Any such line means that player will be missing from the stored roster — investigate and resolve the name before moving on. Also confirm the team count looks right and no roster has a null/undefined player entry.

4. **Reconcile team names against the prior majors:** the same people play every major, but they re-enter their team name each time and often type it differently (capitalization, nickname, `+` vs space, short vs full first name). The roster set should be **identical across all majors** — the prior majors agree with each other, so treat their shared spelling as canonical and rename the odd ones out in the new file. As a general rule, always run this diff after a pull and resolve every mismatch before wiring in. Compare against the existing tournaments:
...
5. **Wire it into the app:** two parallel maps, both keyed off `currentTournament` (decided by month in `src/config/tournaments.ts`, 0-indexed `getMonth()`). A file the app doesn't import shows up as **empty**. Add the new tournament to both:
   - **Rosters** (`src/db/teams.ts`, `teamsByTournament`):
     - `import x from "@data/{year}/$1.json";`
     - add `"$1": x,` to `teamsByTournament` (key by the exact slug).
   - **Rankings** (`src/db/rankings.ts`, `rankingsByTournament`):
     - `import xRankings from "@data/{year}/$1-rankings.json";`
     - add `[TournamentName.<Name>]: xRankings,` to `rankingsByTournament`.

     Skip whichever entry already exists.

6. **Verify:** `npm run check` (fmt + lint + typecheck + tests). The data-integrity test expects the same team-name set across all tournament rosters — a failure there means step 4 missed a mismatch.
